...
async function whatsapp_dialog(frm) {
  ...
  for (var prop in temp) {
    if (
      frappe.meta.has_field(frm.doc.doctype, prop) &&
      frappe.meta.get_field(frm.doc.doctype, prop).options === "Phone"
    ) {
  ...
  var lname = (await frm.doc.first_name)
    ? await frm.doc.first_name
    : await frm.doc.customer_name;
  ...
  let d = new frappe.ui.Dialog({
    ...
    fields: [
      {
        ...
        get_query: function () {
          return {
            filters: [["Aisensy Settings", "enabled_name", "=", 1]],
          };
        },
      },
      {
          ...
          await frappe.call({
            ...
            callback: function (r) {
              if (r && r.message && typeof r.message === "object") {
                ...
                d.set_value(
                  "template_message",
                  template_format + template_footer
                );
                d.set_value(
                  "no_of_template_params",
                  campaign.no_of_template_params
                );
    ...
    primary_action: async function () {
      ...
      if (
        (values.template_type == "IMAGE" ||
          values.template_type == "VIDEO" ||
          values.template_type == "FILE") &&
        (values.url == undefined ||
          values.url == "" ||
          values.url == null ||
          values.file_name == undefined ||
          values.file_name == "" ||
      ...
      var atoken = await frappe.db.get_value(
        "Aisensy Settings",
        values.select_account,
      ...
      if (contact_numbers.length == 0) {
        frappe.msgprint(
          "Please add any contact Number or check the format for adding numbers."
        );
      }
        ...
        await fetch(base_url, {
          ...
          .then(async (response) => {
            if (response.status == 200) {
              ...
              } else valid_numbers += ", " + contact_numbers[i];
              let message =
                "Message moved to sent state, check Aisensy dashboard to for further status. Note: The message may be moved to Failed State, check the dashboard for reasons.";
              message_valid = message;
              frappe.msgprint(
                "For [" + contact_numbers[i] + "], Status 200 : " + message
              );
              return response.ok;
          ...
          .then(async (response) => {
            ...
            if (response.errorMessage == "Invalid Number") {
              ...
              } else invalid_numbers += ", " + contact_numbers[i];
