       ...
       frappe.call({
           ...
           callback: function(r) {
               if (!r.message || r.message.length === 0) {
                   frappe.msgprint("No Aisensy Campaign found for the selected WhatsApp Template.");
                   return;
               }
                   ...
                   let d = new frappe.ui.Dialog({
                       ...
                       primary_action(values) {
                           for (let i = 1; i <= campaign.no_of_template_params; i++) {
                               ...
                               let selected_field = fields.find(f => f.label === selected_label);
                               let fieldname = selected_field ? selected_field.value : selected_label;
