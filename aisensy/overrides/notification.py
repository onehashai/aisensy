...
class AisensyNotification(Notification):
    ...
    def send_aisensy_message(
        self, doc, whatsapp_numbers, campaign, template_parameters
    ):
        ...
        try:
            ...
            message_data = self.prepare_message_data(
                doc, api_key, campaign, template_parameters, whatsapp_numbers
            )
    ...
    def prepare_message_data(
        self, doc, api_key, campaign, template_parameters, whatsapp_numbers
    ):
        ...
        if hasattr(campaign, "location") and campaign.location:
            try:
                ...
                frappe.log_error(
                    "Invalid location format in campaign", campaign.location
                )
    ...
    def make_aisensy_api_call(self, message_data):
        ...
        response = requests.post(
            api_url, headers=headers, json=message_data, timeout=30
        )
